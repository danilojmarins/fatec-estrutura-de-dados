...
void displayReverseListRecursive(struct Node* current, struct LinkedList* list) {
    if (current == NULL)
    {
        return;
    }

    displayReverseListRecursive(current->next, list);
...
void displayReverseList(struct LinkedList* list) {
    ...
    displayReverseListRecursive(list->head, list);
}
